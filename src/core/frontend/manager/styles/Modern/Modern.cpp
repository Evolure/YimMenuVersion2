...
namespace YimMenu
{
	...
	void RenderModernTheme()
	{
		...
		ImVec2 windowPos((*YimMenu::Pointers.ScreenResX - windowWidth) * 0.5f, *YimMenu::Pointers.ScreenResY / 5.0f);
		ImVec2 windowSize(windowWidth, windowHeight);
		const bool useCustomBackground = YimMenu::Menu::customMenuBackground.GetState() && YimMenu::Menu::g_BackgroundTexture.IsValid();
		...
		{
			draw->AddImage(
			    YimMenu::Menu::g_BackgroundTexture.GetTextureID(),
			    winPos,
		...
		float startX =
		    barMin.x +
		    (winW - (bubbleSpacing * submenus.size())) *
		        0.5f;
		...
		for (size_t i = 0; i < submenus.size(); ++i)
		{
			...
			if (isActive)
			{
				draw->AddRectFilled(
				    ...
				    ImVec2(
				        bubblePos.x + bubbleSize,
				        bubblePos.y + bubbleSize
				    ),
				    ToU32(Accent),
				    2.0f
				);
			...
			draw->AddText(
			    YimMenu::Menu::Font::g_AwesomeFont,
			    ICON_SIZE,
			    iconPos,
			    ToU32(
			        isActive
